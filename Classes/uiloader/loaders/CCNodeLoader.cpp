...
cocos2d::CCNode * CCNodeLoader::createObject(rapidjson::Value & config)
{
    return cocos2d::CCNode::create();
}

bool CCNodeLoader::setProperty(cocos2d::CCNode *p, const std::string & name, const rapidjson::Value & value, rapidjson::Value & properties)
{
    if(name == "name")
    {
        if(value.IsString())
